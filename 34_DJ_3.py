...
urlpatterns = [url(r'^$', views.index, name='index'), # 3 Add the 3 views
    ...
    url(r'^(?P<question_id>[0-9]+)/$', views.detail, name='detail'), url(r'^(?P<question_id>[0-9]+)/results/$', views.results, name='results'),
...
{ %
for question in latest_question_list %} < li > < !-- 5
url
defines
directory
to
...
the
namespace
defined in polls / urls.py and the
url
defined in sampsite / urls.py --> < a
...
for the question --> < ul >
{ %
for choice in question.choice_set.all %} < li > < !-- Add
a
s
to
vote if not 1 -->
...
< a
href = "{% url 'polls:detail' question.id %}" > Vote
Again < / a >