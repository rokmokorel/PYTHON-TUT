...
< !-- 1
Aside
from HTML that is defined in your
templates
you
also
need
...
for generating
    ...
    < !-- 1
    Create
    directory / polls / static / polls and define
    style.css
    there -->
    ...
    Create
    the
    directory / polls / static / polls / images and
    save
    a
    repeating
    background
    ...
    { %
    for question in latest_question_list %} < li > < a
    ...
    links in lists.
    Go
    to / polls / templates / polls / index.html
    to
    add
    the
    style
    ...
    TEMPLATES = [{
        'BACKEND': 'django.template.backends.django.DjangoTemplates', # 16 Change 'DIRS': [], to
        'DIRS': [os.path.join(BASE_DIR, 'templates')], # 16 That well tell Django to look in the templates dir
        'APP_DIRS': True,
        'OPTIONS': {
    ...
    { % block
    title %}{{title}} | {{site_title | default: _('Django site admin')}}
    { % endblock %}
    ...
    branding %} < h1
    id = "site-name" > < a
    href = "{% url 'admin:index' %}" > {{site_header | default: _('Django administration')}} < / a > < / h1 >
    { % endblock %}
    -->
